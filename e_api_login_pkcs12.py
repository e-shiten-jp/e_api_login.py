...
# API用ログインidファイル:  "./.pki/e_api_authid.txt"       # ホーム > 「お客様情報」> 「設定情報」画面のAPIでダウンロードしたIDファイル。
# 秘密鍵ファイル:           "./.pki/e_api_private_key.pem"  # 顧客情報画面のAPIでダウンロードした秘密鍵ファイル。
#
# 上記ファイルを用意するための手順
...
#       ファイル名: e_api_authid.txt
# 
# 6）次に「公開鍵暗号化方式公開キー（必須登録）」の「登録（自動）」をオンにします。
# 
# 7）「秘密キー」の欄の下にある「登録」ボタンを押します。公開キーの作成＋登録と秘密鍵の作成が行われます。
# 
# 8）上の操作の直後に「ＤＬ」ボタンを押します。
#   ＊注意：
#       登録ボタンを押したときの画面でのみダウンロード可能になります。
# 　    別の画面に移動してしまうと、後からダウンロードできないので再登録になります。
#   ファイル名：
#       e_api_public_key.pem （ｅ支店に登録される公開鍵。API利用者がプログラムで使うことはありません。）
#       e_api_private_key.pem （ログイン時に返される暗号化された仮想urlを復号するための秘密鍵。絶対に他者に渡さないこと。）
#       e_api_private_key.der （同上、windows用。絶対に他者に渡さないこと。）
#       
#       ※privateキーは、内容を全て理解しているプログラムでのみ使うように心がけてください。
#       ※安全のためにはこのサンプルプログラムも例外ではないです！！　内容を理解してから使ってください。
# 
# 
# 事前準備は以上です。
# 
