...
# =========================================================================
# 暗号化された設定ファイルの保存先
CONFIG_FILE = "./.pki/secure_config.enc"

# 元になる認証情報ファイルの配置先（マニュアル準拠）
FILE_AUTH_ID = "./.pki/e_api_authid.txt"
FILE_PRIVATE_KEY = "./.pki/e_api_private_key.pem"
# =========================================================================
...
def main():
    ...
    # 1. 元になる秘密鍵とAuthIDファイルの読み込み
    try:
        # 認証IDの読み込み
        path_auth_id = Path(FILE_AUTH_ID)
        auth_id = path_auth_id.read_text(encoding="utf-8").strip()
        
        # 秘密鍵の読み込み
        path_private_key = Path(FILE_PRIVATE_KEY)
        pem_text = path_private_key.read_text(encoding="utf-8").strip()

    except FileNotFoundError as e:
        print("\n[エラー] 必要な元ファイルが見つかりません。配置を確認してください。")
        print(f"詳細: {e}")
        print("マニュアルの『3. 準備するファイルの配置』通りに配置されているかご確認ください。")
        return
